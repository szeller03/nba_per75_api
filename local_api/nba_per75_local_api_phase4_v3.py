...
def _load_wowy_stat_layer():
    ...
    if frames:
        ...
        if "Player_ID" in d.columns and d["Player_ID"].notna().any():
            d["__pid_key"]=d["Player_ID"].astype(str).str.strip()
            d["__pid_key"]=d["__pid_key"].where(d["__pid_key"].ne("nan") & d["__pid_key"].ne(""), d["__name_key"])
        else:
            d["__pid_key"]=d["__name_key"]
        d=d.drop_duplicates(["__pid_key","Season"],keep="first")
        if "PER75_Qualified" in d.columns:
            q=d["PER75_Qualified"]
            if q.dtype == object:
                d["PER75_Qualified"]=q.astype(str).str.strip().str.casefold().isin({"1","true","yes","y","qualified"})
            else:
                d["PER75_Qualified"]=q.fillna(False).astype(bool)
        CACHE[key]=d
        return d
...
PLAYOFF_STAT_ALIASES = {
    ...
    "rTS":["rTS","Relative_TS","Relative_TS_pct","rTS_pct","rTS%"],
    "OREB_pct":["OREB_pct","ORB%","ORB_pct","OREB%"],
    "DREB_pct":["DREB_pct","DRB%","DRB_pct","DREB%"],
    "USG_pct":["USG_pct","USG%"],
    "TRB_pct":["TRB_pct","TRB%"],
...
def _load_corrected_playoff_season_from_master():
    ...
    for target,src in stat_sources.items():
        aliases=PLAYOFF_STAT_ALIASES.get(target,[src])
        c=col(df,aliases); out[target]=pd.to_numeric(df[c],errors="coerce") if c else np.nan
    try:
...
def _playoff_long_percentiles(source, career=False):
    ...
    q=(pd.to_numeric(source[gcol],errors="coerce")>=50)&(pd.to_numeric(source[mpcol],errors="coerce")>=1500) if career else ((pd.to_numeric(source[gcol],errors="coerce")>=4)&(pd.to_numeric(source[mpcol],errors="coerce")>=75))
    def ep(y):
        try:y=int(y)
    ...
    def pct(v,higher=True):
        ...
        return out
    lower={"TOV_pct","PF_per75","TOV_per75","DRtg","Relative_DRtg"}
    # Denominator-aware shooting percentages: a player with zero attempts has
    # no defined shooting percentage and must NOT receive a percentile (or a
    # penalty) for that component. This is especially important in playoffs,
    # where small samples are common. The SDI aggregation layer will then
    # renormalize the remaining available statistic weights within the group.
    pct_denoms={"FG_pct":"FGA","2P_pct":"2PA","3P_pct":"3PA","FT_pct":"FTA",
                "TS_pct":"FGA","FTr":"FGA","3PAr":"FGA"}
    for stat in stats:
        value=pd.to_numeric(source[stat],errors="coerce")
        denom_col=pct_denoms.get(stat)
        if denom_col and denom_col in source.columns:
            denom=pd.to_numeric(source[denom_col],errors="coerce")
            value=value.mask(denom<=0)
        b=pd.DataFrame({"Player_ID":source[pid] if pid else source[player],"Player":source[player],"Season":source[season_col] if season_col else np.nan,"G":source[gcol],"MP":source[mpcol],"Statistic":stat,"Value":value,"Qualified":q})
...
def _normalize_playoff_percentile_rows(rows, percentile_field):
    """Add the generic Percentile/Pctl aliases expected by Profile consumers."""
    if rows is None:
        return []
    out=[]
    for row in rows:
        r={k:clean(v) for k,v in row.items()}
        value=r.get(percentile_field) if percentile_field else None
        r["Percentile"]=value
        r["percentile"]=value
        r["Pctl"]=value
        r["pctl"]=value
        r["Percentile_Value"]=value
        out.append(r)
    return out


def _playoff_profile_sdi_payload_v3(percentile_rows, is_career):
    """Return Profile spider axes and Route-A SDI from the active playoff pct family."""
    if not percentile_rows:
        return [], None

    pm=pd.DataFrame(percentile_rows)
    ...
    for a in axes:
        if a.get("value") is None:
            continue
        item=dict(a)
        item["axis"]=label_map.get(str(item.get("axis","")).strip(),str(item.get("axis","")).strip())
        normalized.append(item)

    # Route A: weighted category composite, no re-percentiling.
    spec=_load_sdi_v4_spec()
    top=spec.get("top_level_category_weights",{}) if isinstance(spec,dict) else {}
    raw_axes={str(a.get("axis","")).strip():a.get("value") for a in axes}
    vals={
        "scoring_volume":raw_axes.get("Scoring Volume",raw_axes.get("Scoring")),
        "scoring_efficiency":raw_axes.get("Scoring Efficiency",raw_axes.get("Efficiency")),
        "creation_playmaking":raw_axes.get("Creation & Playmaking",raw_axes.get("Creation / Playmaking")),
        "rebounding":raw_axes.get("Rebounding"),
        "defense":raw_axes.get("Defense"),
        "impact_value":raw_axes.get("Impact & Value",raw_axes.get("Impact / Value")),
    }
    usable=[(float(v),float(top.get(k,0))) for k,v in vals.items()
            if v is not None and pd.notna(v) and float(top.get(k,0))>0]
    sdi=(sum(v*w for v,w in usable)/sum(w for _,w in usable)) if usable else None
    return normalized,sdi
...
def api_playoff_profile(requested, season):
    ...
    if season_df.empty:
        return {
            ...
            "seasons":[],"profile":None,"percentiles":[],
            "category_axes":[],"sdi_v4":None,"SDI_v4":None,
            "SDI_v4_Playoffs":None,"playoff_available":False,
    ...
    if is_career:
        ...
        percentile_field="Career_Percentile"
        percentiles=_normalize_playoff_percentile_rows(
            pm.to_dict("records"), percentile_field
        )
        context_avail={"Season":False,"Era":False,"Historical":False,"Career":True}
        raw_rows=cm
    else:
        sm=player_rows.loc[
            player_rows[season_col].map(_season_label_any).eq(str(requested_view))
        ].copy() if season_col else player_rows.iloc[0:0]

        profile={k:clean(v) for k,v in (sm.iloc[0].to_dict().items() if not sm.empty else [])}

        # Build directly from the authoritative playoff 46-stat season layer.
        # This avoids dependence on a potentially stale regular/legacy percentile CSV.
        per=_playoff_long_percentiles(season_df,career=False)
        ps=_playoff_player_match(per,pid,pname)
        if not ps.empty and "Season" in ps.columns:
            ps=ps.loc[
                ps["Season"].map(_season_label_any).eq(str(requested_view))
            ].copy()

        percentile_field=percentile_column(ps,"Season")
        percentiles=_normalize_playoff_percentile_rows(
            ps.to_dict("records"), percentile_field
        )
    ...
    if not raw_rows.empty:
        ...
        if stat_values.get("2P_pct") is None:
            v=_derive_two_point_pct_from_row(raw_rows)
            if v is not None:
                stat_values["2P_pct"]=v
                profile["2P_pct"]=v

    # Add generic percentile aliases and guarantee the active context is explicit.
    category_axes,sdi_from_pct=_playoff_profile_sdi_payload_v3(percentiles,is_career)

    cached_sdi=None
    if not is_career and requested_view:
        cached_sdi=_playoff_profile_cache_sdi(pid,pname,requested_view)

    sdi=cached_sdi if cached_sdi is not None else sdi_from_pct

    # Profile UI compatibility aliases.
    if sdi is not None and pd.notna(sdi):
        sdi=float(sdi)
        profile["SDI_v4_Playoffs"]=sdi
        profile["SDI_v4"]=sdi
        profile["SDI_v4_WOWY"]=sdi
        if is_career:
            profile["Career_SDI_v4"]=sdi
            profile["Career_SDI_v4_WOWY"]=sdi

    return {
        "found":True,
        "view":requested_view,
        "is_career":is_career,
        "player":{"player_id":pid,"player_name":pname,"headshot_url":None},
        "seasons":seasons,
        "profile":profile,
        ...
        "SDI_v4_Playoffs":sdi,
        "SDI_v4_WOWY":sdi,
        "Career_SDI_v4":sdi if is_career else None,
        "Career_SDI_v4_WOWY":sdi if is_career else None,
        "playoff_sdi_source":(
            "phase3_playoff_sdi_cache" if cached_sdi is not None
            else ("playoff_percentile_profile" if sdi_from_pct is not None else None)
        ),
        "playoff_available":True,
        "playoff_source":str(CACHE.get("__authoritative_playoff_source_path__",PATHS["playoff_46"])),
        "season_type":"Playoffs",
        "available_contexts":context_avail,
        "statistic_registry":PLAYOFF_STATS,
        "career_note":(
            "Career playoff percentiles are computed from the qualified playoff career population. "
            "Single-season playoff percentiles require G >= 4 and MP >= 75."
            if is_career else
            "Single-season playoff percentiles require G >= 4 and MP >= 75."
        ),
    }
...
def _regular_career_big_board(statistic, sort_direction="desc", search=None, limit=100):
    ...
    higher=statistic not in {"TOV_per75","TOV_pct","DRtg","Relative_DRtg"}
...
def _five_year_peak_board(season_type="Regular Season", era=None, statistic=None, sort_direction="desc", search=None, limit=100, anchor_statistic=None):
    ...
    request_cache_key=f"__big_board_peak_v4__:{season_type}:{era or ''}:{statistic}:{sort_direction}:{search or ''}:{int(limit)}"
    ...
    if not rows:
        result={"rows":[],"count":0,"scope":"five_year_peak","season_type":season_type,"era":era or None,"statistic":stat}; CACHE[request_cache_key]=result; return result
    df=pd.DataFrame(rows); lower={"TOV_per75","TOV_pct","DRtg","Relative_DRtg"}; higher=stat not in lower
...
def _sdi_big_board(season=None, context="Historical", sort_direction="desc",
    ...
    if scope_key in {"single","season"}:
        ...
        p = base/fn
        # The locked SDI v4 season index is also shipped in local_api/cache in
        # finalized builds. Prefer the precomputed production file, but use
        # the identical authoritative cache when the convenience copy is not
        # present in a packaged build.
        if not p.exists() and not is_playoff:
            alt = ROOT/"local_api"/"cache"/"regular_sdi_v4_wowy_player_seasons.csv"
            if alt.exists(): p=alt
        if not p.exists() and is_playoff:
            alt = ROOT/"local_api"/"cache"/"playoff_sdi_v4_player_seasons.csv"
            if alt.exists(): p=alt
        if not p.exists():
        ...
        sdi_col = "SDI_v4_WOWY" if (not is_playoff and "SDI_v4_WOWY" in d.columns) else ("SDI_v4" if "SDI_v4" in d.columns else "SDI_v4_WOWY" if "SDI_v4_WOWY" in d.columns else None)
        if not sdi_col:
            return {"rows":[],"count":0,"scope":"single","season_type":season_type,"statistic":"Statistical Dominance Index","note":"SDI v4 source has no SDI score column."}
        d["__sdi_score"]=pd.to_numeric(d[sdi_col],errors="coerce")
        if era:
        ...
        all_score_col = "SDI_v4_WOWY" if (not is_playoff and "SDI_v4_WOWY" in allraw.columns) else ("SDI_v4" if "SDI_v4" in allraw.columns else sdi_col)
        allvals = pd.to_numeric(allraw[all_score_col],errors="coerce").dropna()
        if era:
            ...
            ay=pd.to_numeric(allraw["SeasonEndYear"],errors="coerce")
            allvals=pd.to_numeric(allraw.loc[ay.eq(int(target_year)),all_score_col],errors="coerce").dropna()
...
def _wowy_big_board(season=None, context="Historical", statistic=None, sort_direction="desc", search=None, limit=100, scope="single", season_type="Regular Season", era=None):
    """Build a Big Board directly from the audited individual WOWY layer."""
    w=_load_wowy_stat_layer()
    if w.empty or statistic not in {"WOWY_Offense","WOWY_Defense","WOWY_Net"}:
        return {"rows":[],"count":0,"scope":scope,"season_type":season_type,"statistic":statistic}
    if str(season_type).casefold() not in {"regular season","regular",""}:
        return {"rows":[],"count":0,"scope":scope,"season_type":season_type,"statistic":statistic}
    value_col=statistic
    pct_col=f"{statistic}_Percentile"
    d=w.copy()
    d["__value"]=pd.to_numeric(d.get(value_col),errors="coerce")
    d=d.dropna(subset=["__value"]).copy()
    d["__season_key"]=d["Season"].map(_season_label_any) if "Season" in d.columns else ""
    if era: d=d.loc[d["__season_key"].map(_era_key).eq(str(era).strip())].copy()
    if search: d=d.loc[d["Player"].astype(str).str.contains(str(search),case=False,na=False)].copy()
    # WOWY's audited layer supplies the authoritative PER75 qualification flag.
    if "PER75_Qualified" in d.columns:
        q=d["PER75_Qualified"]
        if q.dtype == object:
            q=q.astype(str).str.strip().str.casefold().isin({"1","true","yes","y","qualified"})
        else:
            q=q.fillna(False).astype(bool)
        # The audited flag is preferred, but an older WOWY cache may contain
        # the values without the flag. In that case retain the valid rows.
        if q.any(): d=d.loc[q].copy()
    if d.empty:
        return {"rows":[],"count":0,"scope":scope,"season_type":season_type,"statistic":statistic}
    if pct_col not in d.columns:
        higher=True
        d["__pct"]=pd.Series(pd.to_numeric(d["__value"],errors="coerce"),index=d.index).rank(method="average",ascending=True)
        n=int(d["__pct"].notna().sum())
        d["__pct"]=100.0 if n<=1 else 100.0*(d["__pct"]-1)/(n-1)
    scope_key=str(scope).casefold().replace("-","_").replace(" ","_")
    if scope_key in {"single","season"}:
        requested=str(season or "").strip()
        if requested.casefold() not in {"","historical","historical percentile","all","all seasons"}:
            target=_season_end_year(requested)
            if target is not None: d=d.loc[d["__season_key"].map(_season_end_year).eq(target)].copy()
        # For historical browsing, each player-season remains an observation.
        if pct_col in d.columns:
            d["__pct"]=pd.to_numeric(d[pct_col],errors="coerce")
        else: d["__pct"]=np.nan
        higher=True
        d=d.sort_values("__value",ascending=(sort_direction=="asc"),kind="stable").head(int(limit)).reset_index(drop=True)
        out=[]
        for i,r in d.iterrows():
            out.append({"rank":i+1,"player_id":clean(r.get("Player_ID")),"player_name":clean(r.get("Player")),"season":clean(r.get("Season")),"season_label":clean(r.get("Season")),"statistic":statistic,"value":clean(r["__value"]),"percentile":clean(r.get("__pct")),"context":context,"headshot_url":_headshot_url_for(clean(r.get("Player_ID")),clean(r.get("Player")))})
        return {"rows":out,"count":len(out),"scope":"single","season_type":"Regular Season","season":season or "Historical Percentile","statistic":statistic,"context":context}
    # Career/era aggregates use the same audited season values and MP weighting.
    if "MP" in d.columns:
        d["__mp"]=pd.to_numeric(d["MP"],errors="coerce").fillna(0)
    else: d["__mp"]=0.0
    if scope_key in {"career","career_average","career_average"}:
        grouped=[]
        for pid,g in d.groupby("Player_ID",dropna=False,sort=False):
            v=g["__value"]; wgt=g["__mp"]
            mask=v.notna() & wgt.gt(0)
            if not mask.any(): continue
            grouped.append({"Player_ID":pid,"Player":str(g["Player"].iloc[0]),"_value_num":float((v[mask]*wgt[mask]).sum()/wgt[mask].sum())})
        d=pd.DataFrame(grouped)
        if d.empty: return {"rows":[],"count":0,"scope":"career","season_type":"Regular Season","statistic":statistic}
        vals=d["_value_num"]; ranks=vals.rank(method="average",ascending=False); n=len(vals); d["__pct"]=100 if n<=1 else 100*(n-ranks)/(n-1)
        d=d.sort_values("_value_num",ascending=(sort_direction=="asc"),kind="stable").head(int(limit)).reset_index(drop=True)
        out=[{"rank":i+1,"player_id":clean(r["Player_ID"]),"player_name":clean(r["Player"]),"season":"Career","season_label":"Career","statistic":statistic,"value":clean(r["_value_num"]),"percentile":clean(r["__pct"]),"context":"Career","headshot_url":_headshot_url_for(clean(r["Player_ID"]),clean(r["Player"]))} for i,r in d.iterrows()]
        return {"rows":out,"count":len(out),"scope":"career","season_type":"Regular Season","season":"Career","statistic":statistic,"context":"Career"}
    # Era average: reuse the same participation gate as the regular Era board,
    # but aggregate only audited WOWY-qualified seasons.
    if scope_key in {"era_average","era"}:
        if not era: return {"rows":[],"count":0,"scope":"era_average","season_type":"Regular Season","statistic":statistic}
        bounds=next(((a,b) for k,a,b,_ in ERA_DEFINITIONS if k==str(era).strip()),(None,None))
        if bounds[0] is None: return {"rows":[],"count":0,"scope":"era_average","season_type":"Regular Season","statistic":statistic}
        d=d.loc[d["__season_key"].map(_season_end_year).between(bounds[0],bounds[1])].copy()
        grouped=[]
        for pid,g in d.groupby("Player_ID",dropna=False,sort=False):
            years=sorted(set(g["__season_key"].map(_season_end_year).dropna().astype(int)))
            eligible=len(range(bounds[0],bounds[1]+1)); q=len(years)
            if not years or q/eligible < ERA_AVERAGE_PARTICIPATION: continue
            if g["__mp"].sum()<ERA_AVERAGE_REGULAR_MIN_MINUTES: continue
            v=g["__value"]; wgt=g["__mp"]; mask=v.notna() & wgt.gt(0)
            if not mask.any(): continue
            grouped.append({"Player_ID":pid,"Player":str(g["Player"].iloc[0]),"_value_num":float((v[mask]*wgt[mask]).sum()/wgt[mask].sum())})
        d=pd.DataFrame(grouped)
        if d.empty: return {"rows":[],"count":0,"scope":"era_average","season_type":"Regular Season","statistic":statistic}
        vals=d["_value_num"]; ranks=vals.rank(method="average",ascending=False); n=len(vals); d["__pct"]=100 if n<=1 else 100*(n-ranks)/(n-1)
        d=d.sort_values("_value_num",ascending=(sort_direction=="asc"),kind="stable").head(int(limit)).reset_index(drop=True)
        out=[{"rank":i+1,"player_id":clean(r["Player_ID"]),"player_name":clean(r["Player"]),"season":"Era Average","season_label":"Era Average","statistic":statistic,"value":clean(r["_value_num"]),"percentile":clean(r["__pct"]),"context":"Era","headshot_url":_headshot_url_for(clean(r["Player_ID"]),clean(r["Player"]))} for i,r in d.iterrows()]
        return {"rows":out,"count":len(out),"scope":"era_average","season_type":"Regular Season","season":"Era Average","statistic":statistic,"context":"Era","era":era}
    return {"rows":[],"count":0,"scope":scope,"season_type":"Regular Season","statistic":statistic}
...
def api_big_board(season=None, context="Historical", statistic=None,
    ...
    if str(statistic or "").strip() in {"WOWY_Offense","WOWY_Defense","WOWY_Net"}:
        return _wowy_big_board(season,context,statistic,sort_direction,search,limit,scope,season_type,era)
    ...
    # WS/48 is a known legacy schema collision: some canonical career/percentile
    # files expose total Win Shares under the literal WS/48 header. For every
    # Big Board scope that still uses the long percentile source, replace that
    # display value with the authoritative rate when it is available.
    if str(statistic or "") in {"WS/48","WS48","WS_per48"} and c_pid and c_season:
        try:
            master=load_master_seasons()
            mpid=col(master,["Player_ID","PlayerId","PlayerID","player_id"]); mname=col(master,["Player","Player_Name","Display_Name","player_name","Name"]); ms=col(master,["Season","season","Season_ID","SeasonEndYear","Season_End_Year"]); mw=col(master,["WS/48.1","WS_per48","WS48_actual"])
            if mpid and ms and mw:
                m=master[[mpid,ms,mw]].copy(); m["__idkey"]=m[mpid].astype(str).str.strip(); m["__seaskey"]=m[ms].map(_season_label_any); m["__ws48_actual"]=pd.to_numeric(m[mw],errors="coerce"); m=m.dropna(subset=["__ws48_actual"]).drop_duplicates(["__idkey","__seaskey"],keep="first")
                lookup={(str(r["__idkey"]),str(r["__seaskey"])):r["__ws48_actual"] for _,r in m.iterrows()}
                work["__ws48_key"]=[(str(r[c_pid]).strip(),_season_label_any(r[c_season])) for _,r in work.iterrows()]
                work["__ws48_actual"]=work["__ws48_key"].map(lookup)
                work.loc[work["__ws48_actual"].notna(),c_value]=work.loc[work["__ws48_actual"].notna(),"__ws48_actual"]
        except Exception:
            pass
...
def _team_analytics_payload():
    if TEAM_ANALYTICS_CACHE.exists():
        try:
            ...
            if p.get("version")==15 and p.get("season_types"):
                return p
        except Exception:
            pass
...
def _preload_big_board_cache():
    ...
    # Build and warm the common Peak boards once. The window population is
    # statistic-independent, while these two metrics cover the default board
    # and its most common companion.
    for _st in ("Regular Season","Playoffs"):
        _five_year_peak_windows(_st)
        for _stat in ("PTS_per75","rTS"):
            try:
                api_big_board("Historical Percentile","Historical",_stat,"desc",None,500,"five_year_peak",_st,None)
            except Exception:
                pass
    # Era qualification is statistic-independent. Warm both the default stat
    # and rTS companion for every regular-season era.
    for _era_key_value,_,_,_ in ERA_DEFINITIONS:
        _era_average_qualified_rows(load_master_seasons(),"Regular Season",_era_key_value)
        for _stat in ("PTS_per75","rTS"):
            try:
                api_big_board(None,"Era",_stat,"desc",None,500,"era_average","Regular Season",_era_key_value)
            except Exception:
                pass
...
if __name__ == "__main__":
    ...
    def warm_caches():
        ...
        try:
            print("Warming public profile percentile caches...")
            warm_public_profile_data()
            print("Public profile percentile caches ready.")
        except Exception as e:
            print("Public profile cache warm failed:", repr(e))
