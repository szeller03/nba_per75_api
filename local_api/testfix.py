...
def _load_sdi_v4_spec():
    ...
    candidates=[
        ...
        ROOT/"local_api"/"config"/"statistical_index_v4_locked.json",
    ]
    spec={}
    for p in candidates:
        if p.exists():
            try:
                ...
                # The locked formula file is a manifest with explicit regular/playoff
                # sections. Normalize the regular section to the internal SDI spec
                # shape used by the regular-season calculation engine.
                if isinstance(raw,dict) and isinstance(raw.get("regular_season"),dict):
                    rs=raw["regular_season"]
                    spec=dict(rs.get("category_formulas",{}) or {})
                    spec["top_level_category_weights"]={
                        str(k):float(v) for k,v in (rs.get("top_level_weights",{}) or {}).items()
                    }
                    spec["top_level_category_total_weight"]=sum(spec["top_level_category_weights"].values())
                    spec["peak_rules"]=rs.get("peak_rules",{}) or {}
                else:
                    spec=raw
                break
            except Exception:
...
def _playoff_season_index_table(source):
    ...
    if isinstance(spec, dict):
        _rows=[]
        for _cat,_groups in spec.items():
            for _grp,_cfg in (_groups or {}).items():
                for _stat,_wt in (_cfg.get("statistics",{}) or {}).items():
                    _rows.append({"Category":_cat,"Group_ID":_grp,"Statistic":_stat,"Statistic_Weight":_wt,"Group_Weight":_cfg.get("weight")})
...
def api_playoff_season_bundles(requested):
    ...
    for _,raw in player_rows.iterrows():
        ...
        try:
            ...
            if playoff_pct is not None and not playoff_pct.empty:
                ...
                if ppid and pscol:
                    pkey=playoff_pct[ppid].astype(str).str.strip().eq(str(pid).strip())
                elif pname_col and pscol:
                    pkey=playoff_pct[pname_col].astype(str).str.replace(r"\*+","",regex=True).str.strip().str.casefold().eq(str(pname or clean(raw.get("Player"))).replace("*","").strip().casefold())
                else:
                    pkey=pd.Series(False,index=playoff_pct.index)
                if pscol:
                    pkey &= playoff_pct[pscol].map(_season_label_any).eq(label)
                matched=playoff_pct.loc[pkey].copy()
                if not matched.empty:
                    # Preserve the established percentile-row shape while
                    # normalizing values to JSON-safe Python numbers.
                    for _,pr in matched.iterrows():
                        item={k:clean(v) for k,v in pr.to_dict().items()}
                        percentile_rows.append(item)
        except Exception:
            # Percentiles are an auxiliary display layer; never allow a
            # percentile failure to replace or corrupt the canonical playoff
            # statistic values in the bundle.
            percentile_rows=[]
    ...
    return {"found":True,"player":player,"seasons":seasons,"rows":rows,
...
def _playoff_sdi_aggregation_spec():
    """Return the playoff projection of the locked regular-season SDI formula.

    This function deliberately mirrors the regular-season v4 specification
    from build_sdi_v4_wowy_v1.py. It does not mutate or replace the regular
    season specification. Playoffs remove Defense and Impact / Value entirely
    and remove only WOWY Offensive Impact from Creation / Playmaking, then
    proportionally renormalize the remaining Creation groups.
    """
    # This is the locked regular-season formula, copied as a read-only local
    # representation because some distributed API packages do not ship the
    # external config CSV/JSON. Keeping this here prevents playoff behavior
    # from silently depending on a missing config file.
    spec={
        "Scoring Volume": {
        ...
        "Scoring Efficiency": {
            ...
            "Component Efficiency": {"weight": .40, "statistics": {"2P_pct": .50, "3P_pct": .40, "FT_pct": .10}},
        },
        "Creation & Playmaking": {
            "Creation Output": {"weight": .385, "statistics": {"AST_per75": .80, "AST_pct": .20}},
            "Ball Security / Creation Cost": {"weight": .315, "statistics": {"AST_TOV": .60, "TOV_pct": .40}},
            "WOWY Offensive Impact": {"weight": .30, "statistics": {"WOWY_Offense": 1.0}},
        },
        "Rebounding": {
            "Rebounding Production": {"weight": .75, "statistics": {"ORB_per75": .45, "DRB_per75": .35, "TRB_per75": .20}},
            "Rebounding Rate": {"weight": .25, "statistics": {"OREB_pct": .45, "DREB_pct": .35, "TRB_pct": .20}},
        },
    }
    spec["Creation & Playmaking"].pop("WOWY Offensive Impact", None)
    for g in spec["Creation & Playmaking"].values():
        g["weight"] /= (.385 + .315)
    return spec
...
def _availability_aware_category_axes(pm, pct_col, playoff=False):
    ...
    if pm is None or pm.empty or not pct_col:
        return []
    spec=_playoff_sdi_aggregation_spec() if playoff else load_exact_csv("aggregation_spec","player_subcategory_aggregation_spec_v1.csv")
    cat=choose_col(spec,["Category"]); grp=choose_col(spec,["Group_ID","Group","Group_Id"])
...
def _percentile_rank_0_100(value, population):
    """Percentile-rank a raw SDI category score on a 0-100 scale."""
    try:
        v=float(value)
    except Exception:
        return None
    ...
    if arr.size==0 or not np.isfinite(v):
        return None
    rank=float((arr < v).sum()) + 0.5*float((arr == v).sum())
    return 100.0 if arr.size==1 else float(100.0*(rank-0.5)/arr.size)


def _regular_season_category_percentiles(season, target_axes):
    """Rank the canonical raw category SDI scores against that season's population."""
    if not target_axes:
        return {}
    ...
    if y is None or not sy:
        return {}
    work=d.copy()
    work["__year"]=pd.to_numeric(work[sy],errors="coerce")
    work=work.loc[work["__year"].eq(int(y))].copy()
    if work.empty:
        return {}
    mapping=[
    ...
    target_by_label={str(x.get("axis") or x.get("label") or ""):x for x in target_axes}
    out={}
    for label,field in mapping:
        if label not in target_by_label or field not in work.columns:
            continue
        raw=target_by_label[label].get("score",target_by_label[label].get("raw_score",target_by_label[label].get("value")))
        vals=pd.to_numeric(work[field],errors="coerce").dropna().to_numpy(dtype=float)
...
def _canonical_regular_season_sdi_axes(pid=None, pname=None, season=None):
    """Return raw regular-season SDI category values from the canonical SDI layer.

    IMPORTANT: this function never substitutes statistic percentiles for SDI
    values. A missing SDI source/row remains unavailable rather than becoming
    a percentile (which can incorrectly display 100 for a player such as Wilt
    or blank a player such as Bill Russell).
    """
    d=_load_canonical_regular_season_sdi_wowy()
    if d.empty or season is None:
        return [], None
    ...
    sy=col(d,["SeasonEndYear","Season_End_Year","Year"])
    sc=col(d,["Season","season","Season_ID"])
    work=d.copy()
    if sy:
        work["__year"]=pd.to_numeric(work[sy],errors="coerce")
    elif sc:
        work["__year"]=work[sc].map(_season_end_year)
    else:
        return [], None
    work=work.loc[work["__year"].eq(int(y))].copy()
    if work.empty:
    ...
    # Prefer canonical player ID, then normalized player name. This handles
    # builds where the website identity table uses a different historical ID
    # namespace from the SDI CSV.
    hit=pd.DataFrame()
    pc=col(work,["Player_ID","PlayerId","PlayerID","player_id"])
    if pid is not None and pc:
        hit=work.loc[work[pc].astype(str).str.strip().eq(str(pid).strip())].copy()

    if hit.empty and pname:
        nc=col(work,["Player","Player_Name","Display_Name","player_name","Name"])
        if nc:
            def _key(v):
                s=unicodedata.normalize("NFKD",str(v or "")).casefold()
                s=re.sub(r"[^a-z0-9]+","",s)
                return s
            wanted=_key(str(pname).replace("*",""))
            hit=work.loc[work[nc].map(_key).eq(wanted)].copy()
    ...
    # Exact player-facing labels. Backend and UI now use "Impact / Value"
    # consistently; "&" remains accepted nowhere in this raw SDI layer.
    mapping=[
        ("Scoring Volume","SDI_scoring_volume"),
        ("Scoring Efficiency","SDI_scoring_efficiency"),
        ("Creation / Playmaking","SDI_creation_playmaking"),
    ...
    ov=pd.to_numeric(r.get("SDI_v4_WOWY",r.get("SDI_v4",np.nan)),errors="coerce")
    return axes, (float(ov) if pd.notna(ov) else None)
...
def _regular_peak_category_population():
    ...
    path=None
    for candidate in (
        "regular_profile_peaks_wowy_canonical_v1.json",
...
def _sdi_big_board(season=None, context="Historical", sort_direction="desc",
    ...
    # formula.
    season_file = None if is_playoff else None
    sdi_index = _load_authoritative_playoff_sdi_v4() if is_playoff else _load_regular_sdi_v4_player_seasons()
