...
CACHE=ROOT/'local_api'/'cache'/'team_competitive_context_v9_sources'
OUT=ROOT/'local_api'/'cache'/'team_competitive_context_v9.json'
REPORT=ROOT/'local_api'/'cache'/'team_competitive_context_v9_build_report.json'
...
def main():
 ...
 result={'version':9,'sources':{'standings':'Land of Basketball browser-rendered year-by-year standings','playoffs':'Basketball-Reference browser-rendered playoffs/series.html','acquisition':'installed Chrome/Edge browser; source HTML cached locally'},'seasons':{}}
 ...
 for i,season in enumerate(seasons,1):
  ...
  if args.refresh or not f.exists():
   try: browser_fetch(browser,url,f,args.visible,args.wait)
    ...
    print('  FAILED:',e)
    if args.interactive_fallback:
     try: interactive_cdp_fetch(browser,url,f,args.wait)
     except Exception as e2: print('  INTERACTIVE FAILED:',e2)
    if not f.exists(): failures.append((season,'standings',str(e)))
