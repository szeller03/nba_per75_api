...
function TeamMark({ item, large = false }: { item?: TeamIndexItem | TeamSeason; large?: boolean }) {
  ...
  if (!src || failed) return <div className={`team-mark-fallback ${large ? 'large' : ''}`}>{String(item?.Team || '?').slice(0, 2).toUpperCase()}</div>;
  return <img className={`team-mark ${large ? 'large' : ''}`} src={src} alt="" onError={() => setFailed(true)} />;
}
  ...
  return <div className="team-page">
    ...
    <section className="team-panel">
      ...
      <div className="team-table">
        <div className="team-row team-row-head"><span>#</span><span>TEAM</span><span>SEASON</span><span>VALUE</span></div>
        {rows.map((r, i) => {
          ...
          return <button className="team-row team-row-button" key={`${team}-${season}-${i}`} onClick={() => onOpenTeam(team)}>
            <span className="rank">{i + 1}</span><span className="team-name"><TeamMark item={item} />{team}</span><span>{season}</span><strong>{signed(value)}</strong>
          </button>;
        })}
      </div>
    </section>
  ...
  return <div className="team-page">
    ...
    <section className="team-panel">
      ...
      <div className="team-table season-table">
        <div className="team-row team-row-head"><span>SEASON</span><span>W–L</span><span>ORtg</span><span>DRtg</span><span>NRtg</span><span>RESULT</span></div>
